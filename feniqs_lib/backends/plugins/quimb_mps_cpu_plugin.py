...
from ..quantum_backends.quimb_mps_backend import QuimbMPSCpuBackend
...
class QuimbMpsCpuPlugin:
    @hookimpl
    def run_backend(self, test_case: str, nb_shots: int, seed: int, **kwargs):
        global_config = update_global_config_from_env()
        return run_backend_common(QuimbMPSCpuBackend, test_case, nb_shots, seed, None, global_config.incl_first, global_config.runs, **kwargs)


def plugin():
    return QuimbMpsCpuPlugin()
