...
from ..quantum_backends.qiskit_simulator_backend import QiskitAerBackend
...
class QiskitAerCpuPlugin:
    @hookimpl
    def run_backend(self, test_case: str, nb_shots: int, seed: int, **kwargs):
        global_config = update_global_config_from_env()
        return run_backend_common(QiskitAerBackend, test_case, nb_shots, seed, None, global_config.incl_first, global_config.runs, **kwargs)

def plugin():
    return QiskitAerCpuPlugin()
