...
from ..quantum_backends.qmatchatea_backend import QMatchaTeaGpuBackend
...
class QmatchateaGpuPlugin:
    @hookimpl
    def run_backend(self, test_case: str, nb_shots: int, seed: int, **kwargs):
        global_config = update_global_config_from_env()
        return run_backend_common(QMatchaTeaGpuBackend, test_case, nb_shots, seed, None, global_config.incl_first, global_config.runs, **kwargs)
 
def plugin():
    return QmatchateaGpuPlugin()
